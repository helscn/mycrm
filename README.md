...
1. 客户评级满足需跟进客户的评级要求；
2. [`Fetch_mail.py`](#fetchmailpy-%e8%84%9a%e6%9c%ac) 脚本监控的个人邮箱中，发件箱中与客户最近一次的发件时间与当前时间大于设定的联系间隔天数；
...
需要注意的是个人邮箱服务器、及账号相关信息是保存在数据库中的，可以前端UI界面进行修改，请参考 [设置](#%e8%ae%be%e7%bd%ae) 中的配置说明。
...
*为了后台持续检查邮箱地址是否有效，需要将此脚本程序加入到系统自动运行计划，根据需要设定间隔时间，比如设定为每小时运行一次获取新邮件消息。*
