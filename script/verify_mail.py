...
logging.basicConfig(
    handlers=[logging.FileHandler(
        encoding='utf-8', mode='a', filename=log_file), logging.StreamHandler()],
...
class IMAP_Client():
    def __init__(self, host="", port=953, user='', password='', ssl=True, tzinfo=8):
        ...
        self.timezone = tzinfo if type(
            tzinfo) is timezone else timezone(timedelta(hours=tzinfo))
        self.__folder = 'INBOX'
    ...
    def get_recent_emails(self, days=30, parts='(RFC822)'):
        ...
        for uid in uids:
            try:
                ...
                logger.error(
                    '获取 {folder} 文件夹中编号为 {uid} 的邮件出现错误！尝试重新连接！'.format(
                        folder=self.__folder or '默认', uid=str(uid))
                )
                conn.logout()
                conn = self.connect()
                typ, data = conn.fetch(uid, parts)
        ...
        while True:
            ...
            try:
                ...
                if retry_count > Max_Retry_Count:
                    sys.stderr.write(
                        'Can not connect to the mail server: {0}\n'.format(self.host))
                    raise e
...
class Email():
            ...
            for addr in header_bytes.split(','):
                ...
                if re.search(r'[a-z_0-9.-]{1,64}@(?:[a-z0-9-]{1,200}.){1,5}[a-z]{1,6}', mailbox, flags=re.M | re.S | re.I):
                    addrs.append((
                        name,
                        re.findall(
                            r'[a-z_0-9.-]{1,64}@(?:[a-z0-9-]{1,200}.){1,5}[a-z]{1,6}', mailbox, flags=re.M | re.S | re.I)[0]
                    ))
                else:
    ...
    def __guess_charset(self, msg):
        ...
        if charset is None:
            ...
            result = re.findall(
                r'(charset\s*=\s*)"?([a-z0-9\-]+)"?', content_type, re.I | re.M | re.S)
            if result:
                charset = result[0][1]
        try:
...
def save_message(cursor, email, msg_type):
    ...
    content = re.sub(r'<!doc[^>]*>|</?html[^>]*>|</?body[^>]*>|<head[^>]*>(.*?)</head[^>]*>|<script[^>]*>(.*?)</script[^>]*>',
                     '', content, flags=re.M | re.S | re.I)
    if not content:
    ...
    if subject == '来自dtn-tech.com的退信':
        ...
        reject_addrs = re.findall(
            r'[a-z_0-9.-]{1,64}@(?:[a-z0-9-]{1,200}.){1,5}[a-z]{1,6}', content, flags=re.M | re.S | re.I)
        ...
        logger.info('正在将 %s 在 %s 发送的邮件 %s 写入数据库，邮件主题为：%s',
                    sender, date, int(email.uid), subject)
...
if __name__ == '__main__':
    try:
        ...
        with DataBase(**config['db']) as cursor:
            ...
            mail_reserved_days = int(get_parameter(
                cursor, 'mail_reserved_days', 365))
            imap_config = {
            ...
            if deleted_rows:
                logger.info('已删除 %s 封距今超过 %s 天的过期邮件...',
                            deleted_rows, mail_reserved_days)
        ...
        for isChild, folder in folders:
            ...
            with DataBase(**config['db']) as cursor:
                ...
                if uids:
                    logger.info('在 %s 文件夹中找到 %s 封新邮件消息，正在保存邮件内容...',
                                folder, len(uids))
